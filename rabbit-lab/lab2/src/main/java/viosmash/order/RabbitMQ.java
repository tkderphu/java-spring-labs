...
public class RabbitMQ {
    ...
    public static  final String ROUTING_KEY_QUANTITY_UPDATED = "routing-key-update-" + DIRECT_EXCHANGE_NEW_ORDER;


    public static final String ORDER_PRODUCT_ROLLBACK_QUEUE = "order-product-rollback";
    public static final String TOPIC_EXCHANGE_ORDER_PRODUCT_ROLLBACK = "order.product.rollback";
    ...
    public Queue orderRollbackQueue() {
        return new Queue(ORDER_PRODUCT_ROLLBACK_QUEUE);
    }
    ...
    @Bean Binding bindingOrderRollbackQueue(Queue orderRollbackQueue, DirectExchange topicExchange) {
        return BindingBuilder.bind(orderRollbackQueue).to(topicExchange).with("order.*.rollback");
    }
